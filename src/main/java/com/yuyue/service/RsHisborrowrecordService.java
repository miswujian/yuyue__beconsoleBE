...
public class RsHisborrowrecordService {
	...
	public Page4Navigator<RsHisborrowrecord> list(int start, int size, int navigatePages, String status, Date starttime,
		...
		Page<RsHisborrowrecord> pageFromJPA;
		if (!returnWay.equals(""))
			pageFromJPA = rsHisborrowrecordDAO
					.queryByStatusInAndstartTimeBetweenAndDeliverTypeInAndReturnWayInAndNikeNameLikeOrVipNoLikeAndBookNameLikeOrRfidLike(
							statuslist, starttime, endtime, deliverTypelist, returnWaylist, "%" + keyword1 + "%",
							"%" + keyword1 + "%", "%" + keyword2 + "%", "%" + keyword2 + "%", pageable);
		else
			pageFromJPA = rsHisborrowrecordDAO
	...
	public void setGenesNull(RsHisborrowrecord rhb) {
		rhb.getBsBookinfo().setBsGenes(null);
	}
	
	public void setGenesNull(List<RsHisborrowrecord> rhbs) {
		for(RsHisborrowrecord rhb : rhbs) {
			setGenesNull(rhb);
		}
	}
